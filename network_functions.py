...
def calculate_network_utilities(net, df_test, full_calculation = False):
    ...
    if not full_calculation:
        ...
        counts = df_utilities_2016.apply(lambda x: possible_outcomes[np.argmax(x)], axis = 1).value_counts()
        counts = counts.reindex(possible_outcomes, fill_value=0)

    return best_options_df, df_utilities_2016, counts, possible_outcomes



def new_screening_strategy(df_test, net, best_options_df, possible_outcomes):
    t1 = time.time()

    df_test["Screening_strategy"] = best_options_df["Best_option"]
    df_test["Colonoscopy"] = None

    for i, strategy in enumerate(possible_outcomes):
        
        if strategy == "No_scr_no_col":
            selected_patients = df_test.loc[best_options_df[best_options_df["Best_option"] == possible_outcomes[i]].index].copy()
...
def old_screening_strategy(df_test, net, best_options_df, possible_outcomes):
